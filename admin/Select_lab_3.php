...
<body>
  ...
  ?>
  <form action="Select_lab_3.php" method="post">

<h4>Select Day And Time Where you want to add LABORATORY Slot in table3</h4></br>
...
<table border="1">
  ...
  if (isset($_POST['reset'])) {
    ...
    if (isset($_SESSION['lab3'])) {
      $_SESSION['lab3'] = array(0,0,0,0,0,0,0);
    }
    ...
    if (isset($_POST['day']) && isset($_POST['time'])){
      if (isset($_POST['submit'])) {
        $day=$_POST['day'];$time=$_POST['time'];$day++;
         $_SESSION['num3'][$time][$day]=1;
        
        for ($i=0; $i <6 ; $i++) { 
          $_SESSION['lab3'][$day]=1;
        }echo "<br>";
           $time++;
          $_SESSION['num3'][$time][$day]=2;
            }
          echo "<tr>";
        for ($col = 0; $col < 7; $col++) {
           ...
           echo "<tr>";
            for ($col = 0,$c=0; $col < 7,$c<7; $col++,$c++) {
                  if ($_SESSION['num3'][$row][$col]==1) {
                    ...
                    else if($_SESSION['num3'][$row][$col]==2){


                            if($_SESSION['num3'][$row][$col]=='2'){   
                               }
                  
                            if ($_SESSION['num3'][$row][$col]=='02:00 To 03:00' && $row==3 && $col==0) {
                             echo "<td colspan=".'2'.">".$_SESSION['num3'][$row][$col]."</td>";
